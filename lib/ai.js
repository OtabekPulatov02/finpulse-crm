...
async function classifyTask(text, categories, opts = {}) {
  ...
  const res = await chat([
    /* лёгкий системник: для классификации полная база знаний не нужна — экономим ~1500 токенов на каждом вызове */
    { role: "system", content: 'Ты — бухгалтер компании Finpulse (Узбекистан). Классифицируй заявку клиента. Ответ строго JSON: {"category": "название из списка или null", "sub": "подкатегория из списка или null", "confidence": 0..1}. Если confidence < 0.6 — category: null.' },
...
{
 "type": "payment_out | payment_in | invoice_esf | act_sverki | schet | postuplenie | realizatsiya | contract | hr_hire | hr_fire | hr_leave | doverennost | letter | report | other",
 "doc1c": "точное название документа 1С",
 "title": "короткое название операции",
 "counterparty": "точное имя контрагента (второй стороны документа — покупателя/поставщика/получателя), как оно упомянуто в заявке — не название своей компании-клиента; если явно назван контрагент, всегда указывай его сюда, а не null",
 "counterpartyInn": "ИНН контрагента или null",
 "counterpartyContract": "название/номер договора с контрагентом, если явно указан в заявке, иначе null",
 "employee": "ФИО сотрудника — заполняй ТОЛЬКО для type hr_hire/hr_fire/hr_leave, как оно упомянуто в заявке, иначе null",
 "hrDate": "YYYY-MM-DD — для hr_leave: дата начала отпуска; для hr_hire: дата приёма; для hr_fire: дата увольнения; иначе null",
 "hrDays": "число дней отпуска — заполняй ТОЛЬКО для type hr_leave, если указано в заявке, иначе null",
 "position": "должность/позиция сотрудника (например \"менеджер по продажам\") — заполняй ТОЛЬКО для type hr_hire, если она упомянута в заявке, иначе null (необязательное поле, не выдумывай, если не указана явно)",
 "amount": число в сумах или null,
 "vat": "12% | без НДС | null",
 "purpose": "назначение платежа/содержание — готовый текст для документа",
 "items": "массив строк товаров/услуг [{\"name\":\"точное наименование товара/услуги как в заявке\",\"qty\":число,\"price\":число,\"amount\":число,\"vat\":\"12%|без НДС\"}] — заполняй ТОЛЬКО для type реализация/поступление/счёт на оплату, если в заявке названы конкретные товары/услуги с количеством и ценой; иначе пустой массив []",
