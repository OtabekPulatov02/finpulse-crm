...
/* Upstash считает КАЖДУЮ команду отдельно для тарифных лимитов — цикл
   с N последовательными redis.set() (как раньше делали синки 1С по
   контрагентам/организациям) означает N команд И N сетевых round-trip'ов.
   mset({key: value, ...}) — ОДНА команда на любое количество ключей,
   в разы дешевле по обеим метрикам. Объект специально (не Map), чтобы
   можно было писать redis.mset({ [`prefix:${id}`]: value }) естественно. */
redis.mset = async (obj) => {
  const entries = Object.entries(obj || {});
  if (!entries.length) return "OK";
  const prefixed = {};
  for (const [k, v] of entries) prefixed[pk(k)] = v;
  return raw.mset(prefixed);
};
