...
const AGENT_TOOLS = [
  ...
  { type: "function", function: { name: "ask_user", description: "Задать уточняющий вопрос пользователю ПЕРЕД выполнением неоднозначного или значимого действия (создание документа в 1С, изменение статуса, удаление, массовая операция и т.п.). Останавливает выполнение до ответа пользователя. Каждый вариант становится отдельной кнопкой в Telegram — можно указать до 8 вариантов, например полный список названий шаблонов документов.", parameters: { type: "object", properties: { question: { type: "string" }, options: { type: "array", items: { type: "string" }, description: "2-8 коротких конкретных варианта ответа" } }, required: ["question", "options"] } } },
  ...
  for (let i = 0; i < 8; i++) {
    ...
    for (const tc of msg.tool_calls) {
      ...
      if (tc.function.name === "ask_user") {
        const options = Array.isArray(args.options) ? args.options.map(String).slice(0, 8) : [];
